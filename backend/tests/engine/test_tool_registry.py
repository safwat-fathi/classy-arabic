...
async def _unavailable(session, action, merchant_id, conversation_id, message_id):
    raise ToolUnavailableError("checkout state requires a Cart which does not exist yet")


async def _echo_product(session, action, merchant_id, conversation_id, message_id):
...
async def test_dispatch_rejects_unapproved_action(db_session, merchant, conversation):
    ...
    rows = (
        (await db_session.execute(select(AIAction).where(AIAction.conversation_id == conversation.id))).scalars().all()
    )
    assert len(rows) == 1

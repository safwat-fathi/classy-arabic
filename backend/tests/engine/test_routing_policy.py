...
def test_no_escalation_when_all_clear():
    reason1 = evaluate_preflight(overflowed=False, correction_count=0, text="عايز اطلب رز")
...
def test_escalates_on_context_overflow():
    reason = evaluate_preflight(overflowed=True, correction_count=0, text="x")
    assert reason == "context_budget_overflow"


def test_escalates_on_repeated_correction():
    reason = evaluate_preflight(overflowed=False, correction_count=2, text="x")
    assert reason == "repeated_correction"


def test_escalates_on_reasoning_heavy_conditional():
    reason = evaluate_preflight(overflowed=False, correction_count=0, text="لو السعر يزيد يبقى هغير الطلب")
...
def test_short_single_question_does_not_escalate():
    ...
    # would send most ordinary customer questions straight to tier 2.
    reason = evaluate_preflight(overflowed=False, correction_count=0, text="الاسعار كام؟")

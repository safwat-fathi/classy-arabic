...
## Summary
- 1605 nodes · 3485 edges · 133 communities (118 shown, 15 thin omitted)
...
Cohesion: 0.06
Nodes (52): _ActionBase, CreateOrderAction, ExtractionResult, GetCheckoutStateAction, GetDeliveryInfoAction, GetProductAction, IntentClassification, json_schema_response_format() (+44 more)
...
Cohesion: 0.15
Nodes (33): ConversationNotFoundError, create_manual_order(), ProductNotFoundError, AsyncSession, Exception, AsyncSession, Decimal, Single, service-owned writer for `Order`/`OrderItem` rows. `status` and… (+25 more)

### Community 3 - "Product"
Cohesion: 0.05
Nodes (116): add_item(), CartItemNotFoundError, _get_item_for_conversation(), _get_or_create_active_cart(), AsyncSession, Exception, line_item_id does not reference an existing cart item for this conversation's…, remove_item() (+108 more)
...
Cohesion: 0.11
Nodes (40): Real delivery-area lookup (SRD §29). A merchant with no configured active areas…, validate_delivery_area(), delete_delivery_area_endpoint(), get_delivery_areas(), patch_delivery_area(), post_delivery_area(), AsyncSession, delete (+32 more)
...
Cohesion: 0.16
Nodes (15): asyncio, embed_text(), find_similar_examples(), AsyncSession, LabeledExample, Base, generate_and_insert(), main() (+7 more)
...
Cohesion: 0.10
Nodes (40): createKnowledgeAction(), createManualOrderAction(), createProductAction(), deleteKnowledgeAction(), deleteProductAction(), fetchKnowledgeAction(), fetchMessagesAction(), fetchProductsAction() (+32 more)

### Community 11 - "process_message"
Cohesion: 0.17
Nodes (34): estimate_cost(), Per-model cost-per-1k-tokens rate table. Empty by default — estimate_cost…, _correction_count(), _known_intents(), _merchant_info(), PipelineResult, process_message(), AsyncSession (+26 more)
...
Cohesion: 0.12
Nodes (33): create(), delete(), get_store_knowledge(), AsyncSession, delete, get, post, put (+25 more)
...
Cohesion: 0.16
Nodes (11): _find_or_create_conversation(), ChannelConnection, Base, Channel, Direction, MerchantStatus, new_id(), seed_knowledge() (+3 more)
...
Cohesion: 0.20
Nodes (23): create(), delete(), get_products(), AsyncSession, delete, get, post, put (+15 more)
...
### Community 98 - "auth/test_router.py"
Cohesion: 0.60
Nodes (5): _mock_facebook_success(), mock, test_facebook_callback_creates_merchant_and_returns_token(), test_facebook_callback_rejects_invalid_facebook_token(), test_facebook_callback_rejects_suspended_merchant()
...
Cohesion: 0.05
Nodes (37): App layout, Architecture, Commands, ⛔ CRITICAL SECURITY RULE: Environment Variables & Secrets, Database/enum gotcha (Alembic), Known in-progress / not-yet-implemented, Testing, Two-stage AI routing (+29 more)

### Community 103 - "worker.py"
Cohesion: 0.14
Nodes (16): AbstractAsyncContextManager, conversation_lock(), ArqRedis, Redis mutex keyed `conversation:{id}:lock` (SRD §36) so two concurrent webhook…, Send a text message to a Facebook/Messenger user via the Send API. Returns True…, send_facebook_reply(), _compose_reply(), _process_channel_message() (+8 more)
...
Cohesion: 0.27
Nodes (12): cluster_messages(), fetch_embedded_messages(), label_cluster(), AsyncSession, representative_messages(), run_clustering(), Message, Base (+4 more)
...
Cohesion: 0.20
Nodes (19): ConversationNotFoundError, AsyncSession, Exception, return_to_ai(), takeover_conversation(), Conversation, _trigger_escalation(), Conversation (+11 more)
...
Cohesion: 0.29
Nodes (5): Merchant, Base, test_known_intents_scoped_to_merchant(), test_merchant_facebook_user_id_allows_multiple_nulls(), test_merchant_facebook_user_id_unique_constraint()

## Knowledge Gaps
- **406 isolated node(s):** `$schema`, `.opencode/plugins/graphify.js`, `WorkerSettings`, `tijaratk-bot`, `MessageRecord` (+401 more)
...
- **Why does `Merchant` connect `Merchant` to `ModelTier`, `Product`, `handoff/router.py`, `DeliveryArea`, `auth/test_router.py`, `resolve_action`, `LabeledExample`, `test_product_matching.py`, `conftest.py`, `Conversation`, `StoreKnowledge`, `ingest_message`, `get_db`, `get_current_merchant`, `conversations/router.py`, `products/service.py`, `app/models/__init__.py`, `auth/router.py`?**
  _High betweenness centrality (0.068) - this node is a cross-community bridge._
- **Why does `Product` connect `Product` to `dispatch_action`, `ModelTier`, `Merchant`, `test_product_matching.py`, `resolve_action`, `LabeledExample`, `process_message`, `get_db`, `app/models/__init__.py`?**
  _High betweenness centrality (0.044) - this node is a cross-community bridge._
- **Why does `process_message()` connect `process_message` to `ModelTier`, `test_product_matching.py`, `resolve_action`, `LabeledExample`, `Message`, `worker.py`, `Conversation`, `StoreKnowledge`, `gateway.py`, `ingest_message`, `extract_order`, `build_context_prompt`, `ConvState`, `match_tier0`?**
